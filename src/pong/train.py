...
from env import make_puffer_pong_env


class PongPolicy(torch.nn.Module):
    """Shared policy/value network for Pong."""
...
class PongEnvFactory:
    """Picklable wrapper for spawning Pong env instances."""

    def __init__(self, render: bool):
        self.render = render

    def __call__(self, buf=None):
        return make_puffer_pong_env(render=self.render, buf=buf)


def build_vecenv(args):
    backend = _backend(args.backend)
    env_factory = PongEnvFactory(args.render)
...
def train(args):
    ...
    policy = PongPolicy(vecenv.driver_env).to(device)
    ...
    try:
        for update in range(total_updates):
            ...
            if (update + 1) % args.save_interval == 0 or (update + 1) == total_updates:
                ...
                ckpt_path = checkpoint_dir / f"pong_ppo_update_{update+1}.pt"
                torch.save(
                    {
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Pong with PufferLib PPO")
    ...
    parser.add_argument(
        "--force-cpu",
        action="store_true",
        help="Disable CUDA/MPS even if available",
    )
